...
class Two_Factor extends Controller {
	...
	public function maybe_redirect_to_show_2fa_enabler() {
		...
		// No use is_user_enabled_otp() because Defender has some 2FA methods since v2.8.0.
	...
	public function alter_user_table_row( $val, $column_name, $user_id ) {
		...
		$class         = 'def-oval';
		$class        .= empty( $provider_slug ) ? '' : ' oval-green';
	...
	public function show_user_profile( $user ) {
		...
		if ( ! $this->service->is_intersected_arrays( $user->roles, $this->model->user_roles ) ) {
			return;
		}
